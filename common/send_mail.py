...
#        mail_body = Run_all_tests.mail_body()
        ...
        try:
            ...
            self.applog.error(u'邮件发送测试报告失败 at'+__file__)
    ...
    def sendReport(self):
        ...
        # 获取最新report的文件夹名称
        day_report_list = os.path.join(gl.report_path, report_list[-2])
 #       print day_report_list
        # 将最新report的文件夹排序
        new_report_list = os.listdir(day_report_list)
        # 得到最新的report
        new_report = new_report_list[-1]
#        print new_report
        # 最新report的绝对路径
        new_report_path = day_report_list+'\\' + new_report
#        print new_report_path
        # 发送邮件
